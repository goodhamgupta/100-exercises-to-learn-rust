...
enum TicketNewError {
    TitleIsEmpty(String),
    TitleIsInvalid(String),
    DescriptionIsEmpty(String),
    DescriptionIsInvalid(String),
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleIsEmpty(msg) | TicketNewError::TitleIsInvalid(msg)) => {
            panic!("Error: {}", msg)
        }
        Err(
            TicketNewError::DescriptionIsEmpty(_msg) | TicketNewError::DescriptionIsInvalid(_msg),
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleIsInvalid(
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionIsInvalid(
