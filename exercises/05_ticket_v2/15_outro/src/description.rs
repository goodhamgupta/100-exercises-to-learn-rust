...
impl TryFrom<String> for TicketDescription {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        if value.is_empty() {
            Result::Err("The description cannot be empty".to_string())
        } else if value.len() > 500 {
            Result::Err("The description cannot be longer than 500 bytes".to_string())
        } else {
...
impl TryFrom<&str> for TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if value.is_empty() {
            ...
            Result::Ok(TicketDescription(value.to_string()))
        }
    }
}
